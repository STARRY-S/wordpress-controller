...
type WordpressLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Wordpress, err error)
...
// List lists all Wordpresses in the indexer.
func (s *wordpressLister) List(selector labels.Selector) (ret []*v1.Wordpress, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Wordpress))
	})
	return ret, err
}
...
type WordpressNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.Wordpress, error)
...
// List lists all Wordpresses in the indexer for a given namespace.
func (s wordpressNamespaceLister) List(selector labels.Selector) (ret []*v1.Wordpress, err error) {
...
// Get retrieves the Wordpress from the indexer for a given namespace and name.
func (s wordpressNamespaceLister) Get(name string) (*v1.Wordpress, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("wordpress"), name)
	}
	return obj.(*v1.Wordpress), nil
}
